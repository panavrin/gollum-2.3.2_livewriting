...
(function($) {
  ...
  $.GollumEditor = function( IncomingOptions ) {
    ...
    $('#gollum-editor-body').livewritingtextarea("create","textarea", {name: "Sang's wiki editor"}, $('#gollum-editor-body').val());

    if ( $("#livewritingActions").val().length > 0)
    {
      $('#gollum-editor-body').livewritingtextarea("playJson", $("#livewritingActions").val());
    }

    $('#gollum-editor-body').livewritingtextarea("register",0, $.GollumEditor.replaceSelectionForReplay0 );
    $('#gollum-editor-body').livewritingtextarea("register",1, $.GollumEditor.replaceSelectionForReplay1 );
    $('#gollum-editor-body').livewritingtextarea("register",2, $.GollumEditor.selectSelectionForReplay2 );
    ...
    if ( EditorHas.baseEditorMarkup() ) {
      ...
      if ( EditorHas.editSummaryMarkup() ) {
        ...
        $('#gollum-editor form[name="gollum-editor"]').submit(function( e ) {
          ...
          var value = $("#gollum-editor-body").livewritingtextarea("returnactiondata");
          if (value){
          
            $('<input>').attr({
                type: 'hidden',
                id: 'livewritingActions',
                name: 'livewritingActions', 
   ...
   */
   var FunctionBar = {
      ...
      executeAction: function( definitionObject ) {
        ...
        if ( repText ) {
          var selPos = FunctionBar.getFieldSelectionPosition( $('#gollum-editor-body') );
          $('#gollum-editor-body').livewritingtextarea("userinput",2,selPos);
          
          FunctionBar.replaceFieldSelection( $('#gollum-editor-body'),
                                             repText, reselect, cursor );
          $('#gollum-editor-body').livewritingtextarea("userinput",1,{"_repText":repText, "_reselect":reselect, "_cursor":cursor});          
  ...
  $.GollumEditor.replaceSelection = function( repText ) {
    ...
    $('#gollum-editor-body').livewritingtextarea("userinput",0,repText);
  };

  
  $.GollumEditor.replaceSelectionForReplay0 = function( repText  ) {
    if(typeof(repText) == "string" ){
        FunctionBar.replaceFieldSelection( $('#gollum-editor-body'),repText  );
        return;
    }
    alert("replaceSelectionForReplay0 needs a string parameters")

  };

  $.GollumEditor.replaceSelectionForReplay1 = function(options) {
    if(typeof(options) == "object" ){
        FunctionBar.replaceFieldSelection( $('#gollum-editor-body'), 
                                  options["_repText"], options["_reselect"], options["_cursor"] );
        return;
    }
    alert("replaceSelectionForReplay1 needs an object parameters")
  };

  $.GollumEditor.selectSelectionForReplay2 = function(selPos){
    if ( $('#gollum-editor-body')[0].setSelectionRange ) {
        $('#gollum-editor-body')[0].setSelectionRange( selPos.start,selPos.end );
      } else if ( $('#gollum-editor-body')[0].createTextRange ) {
        var range = $('#gollum-editor-body')[0].createTextRange();
        range.collapse( true );
        range.moveEnd( 'character', selPos.end );
        range.moveStart( 'character', selPos.start );
        range.select();
      }
  }
